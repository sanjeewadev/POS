...
namespace POS.Cashier.UI.Models
{
    public partial class CartItem : ObservableObject
    {
        ...
        public decimal GrossAmount =>
            MoneyHelper.RoundToAccounting(UnitPrice * Quantity);

        public decimal PercentageDiscountAmount =>
            MoneyHelper.RoundToAccounting(GrossAmount * (DiscountPercentage / 100m));
        ...
        public decimal DiscountAmount
        {
            get
            {
                ...
                return MoneyHelper.RoundToAccounting(discount);
            }
        }

        public decimal LineAmount =>
            MoneyHelper.RoundToAccounting(Math.Max(0m, GrossAmount - DiscountAmount));
        ...
        [ObservableProperty]
        private decimal _vat_amount = 0m;

        [ObservableProperty]
        private decimal _taxInclusiveAmount = 0m;

        public decimal FinalDiscountAmount =>
            MoneyHelper.RoundToAccounting(DiscountAmount + InvoiceDiscountAllocation);

        public decimal FinalLineAmount =>
            IsGiftVoucherSale || IsFreeItem
                ? LineAmount
                : MoneyHelper.RoundToAccounting(TaxInclusiveAmount);

        public decimal CostAmount =>
            MoneyHelper.RoundToAccounting(CostPrice * Quantity);

        public decimal ProfitAmount =>
            MoneyHelper.RoundToAccounting(LineAmount - CostAmount);

        public decimal FinalProfitAmount =>
            MoneyHelper.RoundToAccounting(FinalLineAmount - CostAmount);
        ...
        // (Remaining partials and helper methods unchanged...)
    }
}