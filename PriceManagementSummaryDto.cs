...
namespace POS.Core.Models.DTOs
{
    public class PriceManagementSummaryDto : INotifyPropertyChanged
    {
        ...
        public decimal EffectiveCost => MovingAverageCost > 0m
            ? MoneyHelper.RoundToAccounting(MovingAverageCost)
            : MoneyHelper.RoundToAccounting(LastLandedCost);
        ...
        public decimal NewRetailValue => MoneyHelper.RoundToAccounting(TotalSoh * RetailPrice);
        public decimal? InventoryRetailValue => IsService ? null : NewRetailValue;
        public decimal NewWholesaleValue => MoneyHelper.RoundToAccounting(TotalSoh * WholesalePrice);
        public string ActiveStockRowsText => IsService ? "—" : ActiveStockRowCount <= 0 ? "-" : HasBatchTracking ? ActiveStockRowCount.ToString() : "GENERAL";
        public decimal GrossMarginPercentage => RetailPrice > 0m && EffectiveCost > 0m ? MoneyHelper.RoundToAccounting(((RetailPrice - EffectiveCost) / RetailPrice) * 100m) : 0m;
        public decimal WholesaleMarginPercentage => WholesalePrice > 0m && EffectiveCost > 0m ? MoneyHelper.RoundToAccounting(((WholesalePrice - EffectiveCost) / WholesalePrice) * 100m) : 0m;
        ...
        {
            decimal rounded = MoneyHelper.RoundToAccounting(value);
            if (field == rounded)
                return;
            field = rounded;
    ...
    public class PriceManagementBatchDto : INotifyPropertyChanged
    {
        ...
        public decimal RetailMarginPercentage => EffectiveRetailPrice > 0m && CostPrice > 0m ? MoneyHelper.RoundToAccounting(((EffectiveRetailPrice - CostPrice) / EffectiveRetailPrice) * 100m) : 0m;
        public decimal WholesaleMarginPercentage => EffectiveWholesalePrice > 0m && CostPrice > 0m ? MoneyHelper.RoundToAccounting(((EffectiveWholesalePrice - CostPrice) / EffectiveWholesalePrice) * 100m) : 0m;
