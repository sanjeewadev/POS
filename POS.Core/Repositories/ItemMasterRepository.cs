...
namespace POS.Core.Repositories
{
    ...
    public class ItemMasterRepository
    {
        ...
        public async Task SaveFullMatricesAsync(IEnumerable<(ItemParent Parent, List<ItemVariant> Variants)> matrices)
        {
            if (matrices == null || !matrices.Any()) return;

            var snapshots = new List<(ItemParent Parent, List<ItemVariant> Variants, SubmittedIdentitySnapshot Snapshot)>();

            foreach (var matrix in matrices)
            {
                if (matrix.Parent == null) throw new ArgumentNullException(nameof(matrix.Parent));
                if (matrix.Variants == null || !matrix.Variants.Any()) throw new InvalidOperationException("At least one item variant is required.");

                NormalizeParent(matrix.Parent);
                foreach (var variant in matrix.Variants)
                {
                    NormalizeVariant(variant);
                    if (string.Equals(matrix.Parent.ItemType, ItemTypeCodes.Service, StringComparison.Ordinal))
                    {
            ...
            try
            {
                foreach (var matrix in matrices)
                {
                    var parent = matrix.Parent;
                    var variants = matrix.Variants;

                    if (parent.Id == 0)
                    {
                        string originalItemCode = parent.ItemCode ?? string.Empty;
                        bool isAutoGeneratedCode = originalItemCode.EndsWith("-", StringComparison.Ordinal) || string.IsNullOrWhiteSpace(originalItemCode);
                        if (isAutoGeneratedCode)
                        {
                            parent.ItemCode = await GenerateItemCodeAsync(context);

                            foreach (var variant in variants)
                            {
                                if (variant.SkuCode == originalItemCode || string.IsNullOrWhiteSpace(variant.SkuCode))
                                {
                                    variant.SkuCode = parent.ItemCode;
                                }
                                else if (string.IsNullOrWhiteSpace(originalItemCode) && variant.SkuCode.StartsWith("-", StringComparison.Ordinal))
                                {
                                    variant.SkuCode = parent.ItemCode + variant.SkuCode;
                                }
                                else if (!string.IsNullOrWhiteSpace(originalItemCode) && variant.SkuCode.StartsWith(originalItemCode, StringComparison.OrdinalIgnoreCase))
                                {
                    ...
                    if (parent.Id == 0)
                    {
                        string misalignmentMessage = ItemVariantIdentityPolicy.BuildMisalignmentMessage(parent.ItemCode, variants);
                        if (!string.IsNullOrWhiteSpace(misalignmentMessage)) throw new InvalidOperationException(misalignmentMessage);
                    }

                    var snapshot = SubmittedIdentitySnapshot.Capture(parent, variants);
                    snapshots.Add((parent, variants, snapshot));

                    await ValidateParentReferencesAsync(context, parent);
                    await ValidateItemNameAgainstDatabaseAsync(context, parent.ItemName, parent.Id);
                    ...
                    if (parent.Id == 0)
                    {
                        ...
                        if (!isAutoGeneratedCode)
                        {
                            await ValidateItemCodeAgainstDatabaseAsync(context, parent.ItemCode, parent.Id);
                        }

                        foreach (var variant in variants)
                        {
                            await ValidateSkuAndBarcodeAgainstDatabaseAsync(context, variant, currentVariantId: 0);
                        }

                        ApplyParentActivationStateToSubmittedVariants(parent, variants, parentIsBeingReactivated: false, now);

                        ItemParent persistedParent = CreatePersistedParent(parent, now);

                        await context.ItemParents.AddAsync(persistedParent);
                        await context.SaveChangesAsync(); // We must flush to get the ID for variants
                        ...
                        foreach (var variant in variants)
                        {
                            await AddVariantGraphAsync(context, persistedParent.Id, variant, now);
                        }
                    }
                    else
                    {
                        throw new InvalidOperationException("SaveFullMatricesAsync is strictly for new bulk inserts (Imports).");
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                finally
                {
                    ...
                    {
                        snap.Snapshot.Restore(snap.Parent, snap.Variants);
                    }
                }

                if (ex is InvalidOperationException) throw;
                throw new InvalidOperationException(ItemMasterSaveFailureFormatter.GetUserMessage(ex), ex);
            }
        }
            ...
            List<ItemPropertyMapping> mappings)
        {
            ...
            try
            {
                ...
                if (parent.Id == 0)
                {
                    // The auto-generation was performed earlier.
                    // If the code was manually provided, validate its uniqueness.
                    string originalItemCode = parent.ItemCode ?? string.Empty;
                    bool isAutoGeneratedCode = originalItemCode.EndsWith("-", StringComparison.Ordinal) || string.IsNullOrWhiteSpace(originalItemCode);
                ...
                {
                    // For existing items, the item code cannot be changed.
                    // We still run the uniqueness check here as a final safety measure.
                    await ValidateItemCodeAgainstDatabaseAsync(
                        context,
                        parent.ItemCode,
                        parent.Id);
        ...
        private static async Task ValidateItemNameAgainstDatabaseAsync(
            AppDbContext context,
            string itemName,
            int currentParentId)
        {
            string normalizedName = NormalizeText(itemName);
            string caseInsensitiveCollation =
                DatabaseProviderModelConventions.GetCaseInsensitive(context.Database);

            bool itemNameExists = await context.ItemParents.AnyAsync(parent =>
                EF.Functions.Collate(parent.ItemName, caseInsensitiveCollation) == normalizedName &&
                parent.Id != currentParentId);

            if (itemNameExists)
            {
                throw new InvalidOperationException(
                    $"An item with the name '{itemName}' already exists. Item names must be unique.");
            }
        }
        ...
        public async Task<List<ProductSeekResultDto>> SearchSellableProductsAsync(string searchTerm, string categoryFilter = "ALL CATEGORIES")
        {
            ...
            var rawStock = await context.ItemBatches
                ...
                .Select(b => new { b.ItemVariantId, b.CurrentStock, b.ExpiryDate })
                .ToListAsync();

            var batchDataByVariant = rawStock
            ...
            foreach (var v in variants)
            {
                ...
                if (v.ItemType != ItemTypeCodes.Service && batchDataByVariant.TryGetValue(v.Id, out var batches))
                {
                    stock = batches.Sum(b => b.CurrentStock);
                    earliestExpiry = batches
                        .Where(b => b.ExpiryDate.HasValue && b.CurrentStock > 0)
                        .Select(b => b.ExpiryDate)
                        .OrderBy(d => d)
                        .FirstOrDefault();
                }

                string displayDescription = v.ItemName;
                if (v.VariantDescription != "Standard")
                {
                    displayDescription = $"{v.ItemName} - {v.VariantDescription}";
                }

                results.Add(new ProductSeekResultDto
                {
                    VariantId = v.Id,
                    BatchId = null, // This DTO represents a variant, not a specific batch
        ...
        private async Task<string> GenerateItemCodeAsync(AppDbContext context)
        {
            var sequence = await context.DocumentSequences
                .FirstOrDefaultAsync(d => d.DocumentType == "ITM");
            ...
            while (attempts < 1000)
            {
                ...
                int padding = sequence.PaddingLength <= 0 ? 5 : sequence.PaddingLength;
                ...
                bool exists = await context.ItemParents
                    .AnyAsync(i => EF.Functions.Collate(i.ItemCode, caseInsensitiveCollation) == code);
            ...
            throw new InvalidOperationException("Unable to generate a unique item code.");
        }
    }
}
