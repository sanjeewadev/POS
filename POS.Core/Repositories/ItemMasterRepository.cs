    ...
    public class ItemMasterRepository
    {
            ...
            List<ItemPropertyMapping> mappings)
        {
            ...
            try
            {
                ...
                if (parent.Id == 0)
                {
                    string originalItemCode = parent.ItemCode;
                    // For new items, check if the code needs to be auto-generated.
                    // The convention is that a code ending with a hyphen is a prefix for auto-generation.
                    bool isAutoGeneratedCode = originalItemCode.EndsWith("-", StringComparison.Ordinal);
                    if (isAutoGeneratedCode)
                    {
                        // Append a short, random, unique suffix. This is safer than sequential numbers
                        // as it prevents race conditions without complex database locking.
                        parent.ItemCode = originalItemCode + GenerateRandomSuffix();

                        // The parent ItemCode has been made unique. Now, we must also update the SkuCode
                        // of the variants to match, otherwise they will still be based on the non-unique prefix.
                        foreach (var variant in variants)
                        {
                            // This handles the "Standard" variant where SkuCode was just the prefix.
                            if (variant.SkuCode == originalItemCode)
                            {
                    ...
                    {
                        // If the user provided a full manual code, validate its uniqueness.
        ...
        private static string GenerateRandomSuffix(int length = 6)
        {
            const string chars = "ABCDEFGHJKLMNPQRSTUVWXYZ123456789"; // Avoid ambiguous chars like I, O, 0
